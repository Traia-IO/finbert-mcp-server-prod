#!/usr/bin/env python
"""
FinBERT MCP Server Health Check Script

This script properly connects to the FinBERT MCP server and checks its health by:
...
def check_mcp_server_health(url: str) -> bool:
    """Check if FinBERT MCP server is healthy"""
    print(f"🔍 Checking FinBERT MCP server health at {url}")
    ...
    if "error" in result and "session" not in str(result.get("error", "")):
        ...
        if "result" in result and "tools" in result["result"]:
            ...
            expected_tools = ["finbert_sentiment_analysis", "analyze_financial_text_batch", "get_finbert_model_info"]
            missing_tools = [tool for tool in expected_tools if tool not in tool_names]
            
            if missing_tools:
                print(f"⚠️  Missing expected tools: {', '.join(missing_tools)}")
            else:
                print("✅ All expected FinBERT tools are available!")
...
def main():
    parser = argparse.ArgumentParser(description="Check FinBERT MCP Server Health")
    parser.add_argument("--url", default="http://localhost:8000", 
                       help="FinBERT MCP server URL (default: http://localhost:8000)")
    args = parser.parse_args()
    
    print(f"🚀 FinBERT MCP Server Health Check")
    print(f"📍 Server URL: {args.url}")
    print(f"📰 Expected tools: finbert_sentiment_analysis, analyze_financial_text_batch, get_finbert_model_info")
    print("="*50)
    
    if check_mcp_server_health(args.url):
        print("\n✅ FinBERT MCP Server is healthy and responding!")
        return 0
    else:
        print("\n❌ FinBERT MCP Server health check failed!")
        return 1

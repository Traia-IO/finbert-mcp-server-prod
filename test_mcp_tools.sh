...
# Test script for FinBERT MCP Server tools
# This script tests all available MCP tools to ensure they're working correctly
...
echo -e "${BLUE}🧪 Testing FinBERT MCP Server Tools${NC}"
...
# Test 2: Single text sentiment analysis
echo -e "${BLUE}=== Test 2: Single Text Sentiment Analysis ===${NC}"
...
# Test 3: Another single text analysis (negative sentiment)
...
# Test 7: Error handling - batch with too many texts
echo -e "${BLUE}=== Test 7: Error Handling - Large Batch ===${NC}"
large_batch='['
for i in {1..55}; do
    large_batch+='"Test text number '${i}'"'
    if [ $i -lt 55 ]; then
        large_batch+=', '
    fi
done
large_batch+=']'
make_mcp_call "analyze_financial_text_batch" "{\"texts\": ${large_batch}}" "Test error handling with oversized batch (55 texts)"
echo

# Test 8: Long text truncation
echo -e "${BLUE}=== Test 8: Long Text Truncation ===${NC}"
long_text="This is a very long financial text that should be truncated by the server. $(printf 'A%.0s' {1..600})"
make_mcp_call "finbert_sentiment_analysis" "{\"text\": \"${long_text}\"}" "Test long text truncation"
echo

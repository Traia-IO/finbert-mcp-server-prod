...
# Script to build and run the FinBERT MCP Server locally in Docker
...
IMAGE_NAME="finbert-mcp-server"
CONTAINER_NAME="finbert-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running FinBERT MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# FinBERT MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
# FinBERT doesn't require an API key (payment-only mode)
...
if grep -q "file://" pyproject.toml 2>/dev/null; then
    ...
    if [ -n "$IATP_PATH" ] && [ -d "$IATP_PATH" ]; then
        ...
        if grep -q "file:///tmp/IATP" pyproject.toml 2>/dev/null; then
            if [[ "$OSTYPE" == "darwin"* ]]; then
                sed -i.bak 's|file:///tmp/IATP|traia-iatp>=0.1.66|g' pyproject.toml
            else
                sed -i.bak 's|file:///tmp/IATP|traia-iatp>=0.1.66|g' pyproject.toml
            fi
            docker build --no-cache -t $IMAGE_NAME .
...
echo
echo -e "${GREEN}✅ FinBERT MCP Server is running!${NC}"
echo
